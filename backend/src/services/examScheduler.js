  ...
  try {
    ...
    const { rows: ended } = await client.query(
      `UPDATE exams
       ...
       RETURNING id, title, total_marks, passing_marks`
    );

    if (ended.length) {
      console.log(`[Scheduler] Ended ${ended.length} exam(s):`, ended.map((e) => e.title).join(', '));

      for (const exam of ended) {
        // ── 3. Auto-submit all in_progress submissions for this exam ────────
        const { rows: pending } = await client.query(
          `SELECT es.id AS submission_id, es.student_id, es.exam_id
        ...
        for (const sub of pending) {
          ...
          const percentage = exam.total_marks > 0
            ? parseFloat(((score / exam.total_marks) * 100).toFixed(2))
            : 0;
          ...
          await client.query(
            `UPDATE exam_submissions
             SET status       = 'submitted',
                 ...
                 passed       = $4
             WHERE id = $5`,
            [score, exam.total_marks, percentage, passed, sub.submission_id]
          );
          ...
          await client.query(
            ...
            [sub.student_id, exam.id]
          );
        }

        if (pending.length) {
          console.log(`[Scheduler] Auto-submitted ${pending.length} attempt(s) for "${exam.title}"`);
        }
      }
    }
